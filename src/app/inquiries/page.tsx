...
const columns: ColumnsType<(typeof inquiries)[number]> = [
  { title: '询盘号', dataIndex: 'inquiryNo', render: (value, record) => <Space orientation="vertical" size={0}><Link href={`/inquiries/${record.key}`}><Text strong>{value}</Text></Link><Text type="secondary">{record.receivedAt} · {record.product}</Text></Space> },

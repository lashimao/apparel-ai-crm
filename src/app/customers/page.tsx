...
const columns: ColumnsType<(typeof customers)[number]> = [
  { title: '客户', dataIndex: 'company', render: (value, record) => <Space direction="vertical" size={0}><Link href={`/customers/${record.key}`}><Text strong>{value}</Text></Link><Text type="secondary">{record.country} · {record.source}</Text></Space> },

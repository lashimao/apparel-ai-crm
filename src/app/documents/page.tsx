...
const columns: ColumnsType<(typeof documents)[number]> = [
  { title: '单证号', dataIndex: 'docNo', render: (value, record) => <Space direction="vertical" size={0}><Link href={`/documents/${record.key}`}><Text strong>{value}</Text></Link><Text type="secondary">{record.orderNo}</Text></Space> },

...
const inquiryColumns: ColumnsType<(typeof inquiries)[number]> = [
  { title: '询盘', dataIndex: 'inquiryNo', render: (value, record) => <Space orientation="vertical" size={0}><Text strong>{value}</Text><Text type="secondary">{record.customer}</Text></Space> },
  { title: '国家/来源', render: (_, record) => <Space orientation="vertical" size={0}><Text>{record.country}</Text><Text type="secondary">{record.source}</Text></Space> },
...
const customerColumns: ColumnsType<(typeof customers)[number]> = [
  { title: '客户', dataIndex: 'company', render: (value, record) => <Space orientation="vertical" size={0}><Text strong>{value}</Text><Text type="secondary">{record.country} · {record.value}</Text></Space> },
...
const todoColumns: ColumnsType<(typeof salesCashTodos)[number]> = [
  ...
  { title: '单号/客户', render: (_, record) => <Space orientation="vertical" size={0}><Text strong>{record.no}</Text><Text type="secondary">{record.customer}</Text></Space> },
  ...
  return (
    <AppShell>
      <Space orientation="vertical" size={20} style={{ width: '100%' }}>
        ...
        <Alert showIcon type="warning" icon={<WarningOutlined />} title="有 2 份报价触发毛利底线提醒，AI 建议只能作为草稿，发送前必须人工确认。" action={<Button size="small">查看报价</Button>} />
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} xl={15}>
            <Card variant="borderless" title={<Space><FundProjectionScreenOutlined /> 询盘转化漏斗</Space>} extra={<Button type="link">进入分析</Button>}>
          ...
          <Col xs={24} xl={9}>
            <Card variant="borderless" title={<Space><SafetyCertificateOutlined /> 系统状态</Space>}>
              <Timeline items={[{ icon: <CheckCircleOutlined style={{ color: '#52c41a' }} />, content: '前端框架：角色导航与核心模块已搭建' }, { icon: <CheckCircleOutlined style={{ color: '#52c41a' }} />, content: 'AI 输出统一标记为草稿/建议/风险提示' }, { icon: <ClockCircleOutlined style={{ color: '#faad14' }} />, content: '后端落库接口后续再接入真实数据' }, { icon: <WarningOutlined style={{ color: '#ff4d4f' }} />, content: '正式报价、审批、发送和核销必须人工确认' }]} />
            </Card>
          </Col>
        </Row>

        <Card variant="borderless" title={<Space><DollarOutlined /> 报价 → 订单 → 回款闭环待办</Space>} extra={<Button type="link">进入销售到回款看板</Button>}>
          <Alert style={{ marginBottom: 16 }} type="warning" showIcon title="正式价格、低毛利审批、报价发送和回款核销均不得由 AI 自动生效，必须进入人工确认/审批链。" />
          <Table columns={todoColumns} dataSource={salesCashTodos} pagination={false} scroll={{ x: 900 }} />
        </Card>

        <Row gutter={[16, 16]}>
          <Col xs={24} xl={15}><Card variant="borderless" title={<Space><MailOutlined /> 今日重点询盘</Space>} extra={<Button icon={<SendOutlined />}>批量分配</Button>}><Table columns={inquiryColumns} dataSource={inquiries} pagination={false} scroll={{ x: 900 }} /></Card></Col>
          <Col xs={24} xl={9}><Card variant="borderless" title={<Space><RobotOutlined /> AI 审核队列</Space>}><Space orientation="vertical" size={12} style={{ width: '100%' }}>{aiTasks.map((task) => <div key={task.key} className="crm-card-soft"><Space align="start"><Button shape="circle" icon={task.icon} /><Space orientation="vertical" size={0} style={{ flex: 1 }}><Text strong>{task.title}</Text><Text type="secondary">{task.desc}</Text></Space><StatusTag value={task.status} /></Space></div>)}</Space></Card></Col>
        </Row>

        <Row gutter={[16, 16]}>
          <Col xs={24} xl={15}><Card variant="borderless" title={<Space><TeamOutlined /> 关键客户提醒</Space>}><Table columns={customerColumns} dataSource={customers} pagination={false} scroll={{ x: 900 }} /></Card></Col>
          <Col xs={24} xl={9}><Card variant="borderless" title={<Space><ThunderboltOutlined /> 角色工作台</Space>}><Space orientation="vertical" size={12} style={{ width: '100%' }}>{roleCards.map((role) => <div key={role.role} className="crm-card-soft"><Text strong>{role.role}</Text><br /><Text type="secondary">{role.focus}</Text><div style={{ marginTop: 8 }}>{role.todos.map((todo) => <Tag key={todo}>{todo}</Tag>)}</div></div>)}</Space></Card></Col>
        </Row>
      </Space>
    </AppShell>
  )

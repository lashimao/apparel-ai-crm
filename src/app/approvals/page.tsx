...
const columns: ColumnsType<(typeof approvals)[number]> = [
  { title: '审批类型', dataIndex: 'type', render: (value, record) => <Space direction="vertical" size={0}><Text strong>{value}</Text><Text type="secondary">{record.refNo}</Text></Space> },
...
export default function ApprovalsPage() {
  return <AppShell><Space direction="vertical" size={20} style={{ width: '100%' }}><PageHeader title="审批中心" subtitle="低毛利报价、异常账期、特殊折扣和交付例外审批" actions={<Button type="primary" icon={<AuditOutlined />}>审批规则</Button>} /><Alert showIcon type="warning" message="审批通过只代表允许进入下一步，仍需业务员人工发送报价或确认订单。" /><Row gutter={[16, 16]}><Col xs={24} xl={17}><Card bordered={false} title="待审批事项"><Table columns={columns} dataSource={approvals} scroll={{ x: 1100 }} /></Card></Col><Col xs={24} xl={7}><Card bordered={false} title="审批原则"><Space direction="vertical" size={12}><div className="crm-card-soft"><Text strong>低毛利</Text><br /><Text type="secondary">低于底线必须说明原因，并记录审批人。</Text></div><div className="crm-card-soft"><Text strong>异常账期</Text><br /><Text type="secondary">影响现金流的账期调整必须管理层确认。</Text></div><div className="crm-card-soft"><Text strong>全程审计</Text><br /><Text type="secondary">提交、通过、拒绝和撤回均要写入操作日志。</Text></div></Space></Card></Col></Row></Space></AppShell>
}

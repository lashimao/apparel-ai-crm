...
const columns: ColumnsType<(typeof orders)[number]> = [
  { title: '订单号', dataIndex: 'orderNo', render: (value, record) => <Space orientation="vertical" size={0}><Link href={`/orders/${record.key}`}><Text strong>{value}</Text></Link><Text type="secondary">{record.customer}</Text></Space> },
...
export default function OrdersPage() {
  return <AppShell><Space orientation="vertical" size={20} style={{ width: '100%' }}><PageHeader title="订单中心" subtitle="报价转订单、定金、生产、质检、发货和完结状态追踪" actions={<Button type="primary" icon={<ShoppingCartOutlined />}>新建订单</Button>} /><Row gutter={[16, 16]}><Col xs={24} xl={17}><Card variant="borderless" title="订单列表"><Table columns={columns} dataSource={orders} scroll={{ x: 1100 }} /></Card></Col><Col xs={24} xl={7}><Card variant="borderless" title="订单履约流程"><Steps orientation="vertical" current={2} items={[{ title: '待确认' }, { title: '已确认' }, { title: '定金已收' }, { title: '生产中' }, { title: '待发货' }, { title: '已发货' }, { title: '已完成' }]} /></Card></Col></Row></Space></AppShell>
}

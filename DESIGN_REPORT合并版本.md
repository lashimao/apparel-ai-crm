...
/**
 * 数据权限层级：
 * - sales: 只能看自己负责的客户/询盘/报价
 * - manager: 能看本团队所有数据
 * - admin: 能看所有数据
 * - finance: 能看所有订单和财务数据，不能看客户联系方式（脱敏）
 */
export const DATA_PERMISSION_RULES = {
  sales: {
  ...
  manager: {
    customer: { filter: 'team_id IN (SELECT id FROM teams WHERE manager_id = :userId)' },
    inquiry: { filter: 'team_id IN (SELECT id FROM teams WHERE manager_id = :userId)' },
  },
  admin: {
    customer: { filter: null }, // 不加过滤
    inquiry: { filter: null },
  },
  finance: {
    customer: { filter: null, maskedFields: ['email', 'phone', 'whatsapp'] },
    order: { filter: null },
    payment: { filter: null },
  },
}
